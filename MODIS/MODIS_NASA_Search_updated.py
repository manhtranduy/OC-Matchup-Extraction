...
class MODIS_NASA_Search:
    ...
    def handle_spatial_input(self):
        if isinstance(self.Input, list):
            if len(self.Input) > 2:
                ...
                self.spatial_input = f'bounding_box[]={self.Input[0]:.4f},{self.Input[1]:.4f},{self.Input[2]:.4f},{self.Input[3]:.4f}'
    ...
    def product_info(self, product):
        ...
        if 'C1379762888-LAADS' in self.datacollection:
            ...
            # https://oceandata.sci.gsfc.nasa.gov/getfile/A2022280053000.L1A_LAC.bz2
        ...
        elif 'C1570116979-OB_DAAC' in self.datacollection or self.datacollection=='C1570116979-OB_DAAC':
            # pass

            # product_summary['product_list']=product['producer_granule_id']
            product_summary['product_id']=product['producer_granule_id']
            product_summary['collection']=product['collection_concept_id']
            product_summary['product_link']='https://oceandata.sci.gsfc.nasa.gov/getfile/{}.bz2'.format(
                       product_summary['product_id'])
            product_summary['satellite_datetime']=product['time_end']
            product_summary['day_night_flag']=product['day_night_flag']
